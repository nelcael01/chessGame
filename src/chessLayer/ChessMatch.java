...
import chessLayer.picies.King;
import chessLayer.picies.Rook;
...
public class ChessMatch {
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        if (capturesPiece != null) {
            piecesOnTheBoard.remove(capturesPiece);
            capturedPieces.add(capturesPiece);
        }
    ...
    private void validateSourcePosition(Position position) {
        if (!board.thereIsAPiece(position)) {
            throw new ChessException("There is no piece ou source position");
        }
        if (currentPlayer != ((ChessPiece) board.piece(position)).getColor()) {
            throw new ChessException("The chosen piece is not yours");
        }
        if (!board.piece(position).isThereAnyPossibleMove()) {
            throw new ChessException("There is no possible moves for the closen piece");
        }
    }
